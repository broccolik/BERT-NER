...
flags.DEFINE_string(
    "trans_model_dir", '/home/zhanghongkuan/BERT-NER-CRF/output/saved_model',
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        with open('mask.txt', 'a', encoding='utf-8')as f1:
            f1.write(str(mask) + '\n\n')
...
def get_words_and_labels_for_docs(model_output_dir):
    '''
    从模型预测结果中加载word和label,针对文档集合
    :return:words, labels
    '''
    with open(os.path.join(model_output_dir, 'label_test.txt'), 'r', encoding='utf-8')as f1:
        words = []
        labels = []
        lines = []
        for line in f1:
...
def get_entity_for_docs(model_output_dir):
    '''
    从文档集合中提取出实体,保存至entity_test.json
    :return:
    '''
    lines = get_words_and_labels_for_docs(model_output_dir)
    f2 = open(os.path.join(model_output_dir, 'entity_test.json'), 'w', encoding='utf-8')
    entity = ''
    type_ = ''
    entity_list = []
    for item in lines:
        ...
        for word, tag in zip(word_list, tag_list):
            ...
            if entity != '' and type != '' and str(tag) == 'O':
                if type_ not in b_dict.keys():
                    b_dict[type_] = entity
                entity = ''
                type_ = ''
        b_list.append(b_dict)
        a_dict['text'] = text
        a_dict['entity'] = b_dict
        entity_list.append(a_dict)
    json.dump(entity_list, f2, ensure_ascii=False, indent=3)
    # print(entity_list)
    f2.close()
    return entity_list
...
def serving_input_fn():
    ...
    label_ids = tf.placeholder(tf.int32, [None, 200], name='label_ids')
    input_ids = tf.placeholder(tf.int32, [None, 200], name='input_ids')
    mask = tf.placeholder(tf.int32, [None, 200], name='mask')
